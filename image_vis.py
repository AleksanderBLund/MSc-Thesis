...
class InteractiveViewer:
    ...
    def save_image(self, event=None):
        ...
        # Gather all axes in the figure and identify the main image axis (self.ax)
        all_axes = self.fig.get_axes()
        main_ax = self.ax

        # Identify extra control axes (all axes except main_ax)
        control_axes = [ax for ax in all_axes if ax is not main_ax]

        # Save previous visibility states of these axes
        prev_visibility = {ax: ax.get_visible() for ax in control_axes}
        for ax in control_axes:
            ax.set_visible(False)

        # Remove ticks and spines from the main axis
        main_ax.set_xticks([])
        main_ax.set_yticks([])
        for spine in main_ax.spines.values():
            spine.set_visible(False)

        # Save the figure with only the main image visible, using tight bounding box
        self.fig.savefig(file_path, format='pdf', bbox_inches='tight')

        # Restore the control axes
        for ax in control_axes:
            ax.set_visible(prev_visibility[ax])
    ...
    def launch(self):
        ...
        if self.structure_lines:
            ...
            self.legend_obj = self.ax.legend(by_label.values(), by_label.keys(), loc='upper right', fontsize=12)
...
class GBMViewerApp(tk.Tk):
    ...
    def __init__(self, patients, modalities):
        ...
        # Instance variable to store the currently loaded viewer
        ...
        # Top frame for dropdown menus and buttons
        ...
        # Add the new Save Image button beside Load Viewer
        save_button = tk.Button(control_frame, text="Save Image", command=self.save_current_image)
        save_button.pack(side=tk.LEFT, padx=10)

        # Frame for matplotlib canvas
        self.canvas_frame = tk.Frame(self)
        self.canvas_frame.pack(fill=tk.BOTH, expand=True)
        #remove grid in photo. dont want any grids
    ...
    def load_viewer(self):
        ...
        # Store the interactive viewer for later use
        self.current_viewer = processor.viewer
    ...
    def save_current_image(self):
        # Ensure there is a current viewer loaded
        if self.current_viewer is None:
            print("No image to save. Please load a viewer first.")
            return
        # Call the save_image method of the current viewer
        self.current_viewer.save_image()
